...
def forward(data, label, params, dimensions):
    ...
    # Forward pass through hidden layer with sigmoid activation
    h = sigmoid(data @ W1 + b1)
    # Forward pass through output layer with softmax activation
    y_hat = softmax(h @ W2 + b2)
    # Return probability for the correct label
...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    ### YOUR CODE HERE: forward propagation
    # Forward pass
    h = sigmoid(data @ W1 + b1)  # Hidden layer activation
    y_hat = softmax(h @ W2 + b2)  # Output probabilities
    ...
    grad = np.concatenate(
        (gradW1.flatten(), gradb1.flatten(), gradW2.flatten(), gradb2.flatten())
    )
...
def sanity_check():
    ...
    gradcheck_naive(
        lambda params: forward_backward_prop(data, labels, params, dimensions), params
    )
